...
**VF-9.2 Chute-cap ✅ (2026-07-10)**: сквозной Ø9.4 бор cap читался как
«тоннель» и не имел упора для трубки → **вариант B (утв.)**: socket с упорным
буртиком (глухое дно) → orifice Ø5 → короткая камера (≤10мм) → **открытый
сверху носик-жёлоб** (U-корыто из рёбер), капля на 4.5мм внутрь канала
(DRIP_INSET, парный сдвиг датумов feed/spout — поза ряда не меняется). Правило
пользователя = валидатор **`form.cap_water_path_visible`** (закрытый
горизонтальный водный тоннель = FAIL). `hose_bore_ok` инвертирован (socket
ОБЯЗАН быть глухим), `fluid_path_open` — составная полилинейная проба,
`no_standing_water_ir` — экземпшен «глухой бор дренируется соосным orifice»,
новый assembly `cap_chute_drains_under_mount` (дренаж жёлоба в ПОЗЕ
