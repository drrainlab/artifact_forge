# Vertical Farm Pack — канон раздела
...
> признана **design mistake** для целевого продукта и заменена каноном
> **tilted_flush_row**: flush-модули без Z-step, желоб постоянной глубины,
> уклон создаётся МОНТАЖОМ ряда на стандартный прямой 2020/3030 профиль
> (mount_context, 1.0–2.0°), межмодульная передача — lap-flow стык,
> магниты — только выравнивание. Каскадные goldens удалены (история в
> git: VF-3/VF-4 коммиты); drip-handover остался только у адаптеров
> (cap → первый rail, последний rail → collector). Разделы ниже,
> противоречащие этому, считаются superseded до полной ревизии документа.
