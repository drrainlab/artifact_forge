...
## Lightweight dry shell (в опе; generic-модификатор — следующая волна)

- Окна: открытые снизу CutBox'ы в слэбе под seat-полом, слева/справа от
  желоба; крыша ≥ 2.4 под seat-полом; рёбра 2.0 между окнами; продольные
  band'ы под пазы профиля и периметр нетронуты; запретные зоны (желоб ±2,
  lap-концы, магниты, коридоры) — `form.lightweight_windows_dry_ok`.
...
- Обратимо: `lightweight: false` → сплошная плита (smoke-тест держит оба
  варианта). Экономия на дефолтной ячейке: 1068 → 639 см³ (−40%).
- `lightweight_dry_shell_v1` как переиспользуемый модификатор (field-кернел,
  forbidden regions, weight_saving_target) — отдельная волна (ROADMAP).
...
- **VF-4.1**: anti-slide удержание ряда на СМОНТИРОВАННОМ под уклоном
  профиле (клипсы/упоры) — посадка теперь полная, но продольного замка нет;
