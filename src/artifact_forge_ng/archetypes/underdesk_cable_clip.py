...
hook below). Screw axis is Z; screws are spaced along Y.
"""
...
def build_form(
    ...
    width = ctx["flange_w"]  # hook width along X == flange width
    ...
    ys = _screw_positions(ctx)
    holes = [
        HoleFeature(at=(width / 2.0, y, flange.z_top), screw=screw, through=ft)
        for y in ys
    ]
    ...
    keepouts = [
        Region2D(
            ...
            shape=Circle2D(Pt(width / 2.0, y), head_r + KEEPOUT_CLEARANCE),
            clearance=0.0,
        )
    ...
    if hex_use is not None:
        ...
        if raw_cell is not None:
            ...
            if v.kind == "literal" and v.literal:
                cell = v.literal
        window = Rect2D(0.0, -fl / 2.0, width, fl / 2.0)
        fields.append(
    ...
    regions = [
        Region("flange", RegionRole.MOUNTING_SURFACE,
               Box3(0.0, -fl / 2.0, 0.0, width, fl / 2.0, flange.z_top)),
        Region("screw_zones", RegionRole.FASTENER_KEEPOUT,
               Box3(width / 2.0 - head_r - KEEPOUT_CLEARANCE, min(ys) - head_r - KEEPOUT_CLEARANCE, 0.0,
                    width / 2.0 + head_r + KEEPOUT_CLEARANCE, max(ys) + head_r + KEEPOUT_CLEARANCE, flange.z_top)),
        ...
        Region("perforation_safe_zone", RegionRole.AESTHETIC_LIGHTENING,
               Box3(0.0, -fl / 2.0, flange.z_top - ft, width, fl / 2.0, flange.z_top)),
    ]
