...
def test_horizontal_round_bore_warns():
    form = make_form(bores=[BoreFeature("drain", axis="Y", center=(0, 0, 15),
                                        d=9.4, span=(-60, 60))])
    f = horizontal_bore_supportless(solid_with(), form)
    assert f.status is Status.WARN
    assert "teardrop" in f.message


def test_horizontal_teardrop_bore_passes():
    form = make_form(bores=[BoreFeature("drain", axis="Y", center=(0, 0, 15),
                                        d=9.4, span=(-60, 60), roof="teardrop")])
    f = horizontal_bore_supportless(solid_with(), form)
    assert f.status is Status.PASS
    assert "self-supporting" in f.message


def test_small_or_vertical_bores_are_na():
    form = make_form(bores=[
        BoreFeature("small", axis="Y", center=(0, 0, 15), d=6.0, span=(-60, 60)),
        BoreFeature("vert", axis="Z", center=(30, 30, 0), d=12.0, span=(0, 30)),
    ])
    f = horizontal_bore_supportless(solid_with(), form)
    assert f.status is Status.PASS
    assert "n/a" in f.message
